...
import com.hngy.po.*;
import com.hngy.service.*;
import com.hngy.utils.Consts;
import com.hngy.utils.Pager;
...
public class ItemOrderController extends BaseController {
    ...
    public String exAdd(@RequestBody List<CarDto> list,HttpServletRequest request){
        ...
        for(CarDto c:list){
            ...
            if(c.getNum()!=0){
                String sql="update item set stock =stock-1 WHERE id = "+load.getItemId()+" ";
                for(int i=0;i<c.getNum();i++){
                    itemService.updateBysql(sql);
                }
            }
            System.out.println("aaaaaaaaaaaaaaaaaaaa="+load.getItemId());
        }
